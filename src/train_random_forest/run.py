...
def go(args):
    ...
    cate_columns = [i for i in processed_features if X_val_temp[i].dtype == object]
    X_val_temp[cate_columns] = X_val_temp[cate_columns].astype('category')

    with open('temp_file_4.txt','w') as t:
        t.write(' '.join([f'{i}: {X_val_temp[i].dtype}'for i in processed_features]))
    signature = infer_signature(X_val_temp[processed_features], y_pred)
